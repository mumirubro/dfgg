...
def parse_proxy_format(proxy_string):
    """
    Parse proxy from ALL formats and return httpx-compatible URL:
    
    Supported formats:
    - host:port:user:pass -> http://user:pass@host:port
    - user:pass@host:port -> http://user:pass@host:port
    - user:pass:host:port -> http://user:pass@host:port
    - http://user:pass@host:port -> (return as-is)
    - https://user:pass@host:port -> (return as-is)
    - socks5://user:pass@host:port -> (return as-is)
    - socks4://host:port -> (return as-is)
    ...
    if proxy_string.startswith(('http://', 'https://', 'socks5://', 'socks4://', 'socks5h://')):
        return proxy_string
    ...
    if len(parts) == 4:
        ...
        if part4.isdigit() and len(part4) <= 5:
...
def get_next_proxy():
    ...
    raw_proxy = GLOBAL_SETTINGS['proxies'][GLOBAL_SETTINGS['proxy_index']]
    GLOBAL_SETTINGS['proxy_index'] = (GLOBAL_SETTINGS['proxy_index'] + 1) % len(GLOBAL_SETTINGS['proxies'])
    save_settings()
    
    formatted_proxy = parse_proxy_format(raw_proxy)
    logger.info(f"Using proxy: {formatted_proxy[:50] if formatted_proxy else 'None'}...")
...
async def addp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "• host:port\n\n"
            "Example:\n"
            "/addp proxy.example.com:5959:myuser:mypass"
        )
        return
    ...
    if proxy not in GLOBAL_SETTINGS['proxies']:
        ...
        save_settings()
        formatted = parse_proxy_format(proxy)
        await update.message.reply_text(
            f"✅ Proxy added!\n"
            f"🔌 Raw: {proxy[:50]}...\n"
            f"🔗 Formatted: {formatted[:50] if formatted else 'N/A'}...\n\n"
            f"📊 Total proxies: {len(GLOBAL_SETTINGS['proxies'])}"
        )
    else:
...
async def cp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with httpx.AsyncClient(proxy=formatted_proxy, timeout=15.0) as client:
