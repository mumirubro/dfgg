...
- **Implemented Netflix Account Checker:**
  - Added `/net` command for single Netflix account check (email:password format)
  - Added `/mnet` command for mass Netflix checking with 25 concurrent workers
  - Added `/pnet` command (admin only) to set proxy for Netflix checker
  - Uses Playwright browser automation for accurate login verification
  - Supports all proxy formats: host:port:user:pass, user:pass:host:port, protocol://user:pass@host:port, socks5://, socks4://
  - Max 5000 accounts for regular users, unlimited for admins
  - Generates output files for hits, dead accounts, and errors
  - Added Netflix to account checker menu in /cmd
